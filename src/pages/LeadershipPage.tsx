...
function ContactDetails({ member }: { member: TeamMember }) {
  const contacts = [
    { label: member.linkedin, icon: BriefcaseBusiness },
    { label: member.email, icon: Mail },
    { label: member.whatsapp, icon: MessageCircle },
  ]

  return (
    <div className="mt-5 grid gap-2 sm:grid-cols-3">
      {contacts.map(({ label, icon: Icon }) => (
        <a className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[#D4E8DD] bg-[#edf7f2] px-3 text-sm font-bold text-[#0B6E4F]" href="#" key={label}>
